...
def composite(y, dx=1, order=3):
    ...
    coef_dic = {
        1: np.array([-1]) / 2.0,
        2: np.array([-15, 4, -1]) / 24.0,
        3: np.array([-16, 7, -4, 1]) / 24.0,
        4: np.array([-965, 462, -336, 146, -27]) / 1440.0,
        5: np.array([-981, 542, -496, 306, -107, 16]) / 1440.0,
        6: np.array([-84161, 55688, -66109, 57024, -31523, 9976, -1375]) / 120960.0,
        7: np.array([-85376, 64193, -91624, 99549, -74048, 35491, -9880, 1215])
        / 120960.0,
    }
...
def cumulative_composite(y, dx=1, initial=[], order=3):
    ...
    # integration coefficients
    coef_dic = {
        1: (np.array([[]]), np.array([1, 1]) / 2.0),
        2: (np.array([[5, 8, -1]]) / 12.0, np.array([-1, 13, 13, -1]) / 24.0),
        3: (np.array([[9, 19, -5, 1]]) / 24.0, np.array([-1, 13, 13, -1]) / 24.0),
        4: (
            np.array([[251, 646, -264, 106, -19], [-19, 346, 456, -74, 11]]) / 720.0,
            np.array([11, -93, 802, 802, -93, 11]) / 1440.0,
        ),
        5: (
            np.array(
                [[475, 1427, -798, 482, -173, 27], [-27, 637, 1022, -258, 77, -11]]
            )
            / 1440.0,
            np.array([11, -93, 802, 802, -93, 11]) / 1440.0,
        ),
        6: (
            ...
            / 60480.0,
            np.array([-191, 1879, -9531, 68323, 68323, -9531, 1879, -191]) / 120960.0,
        ),
        7: (
            np.array(
                [
                    ...
                    [351, -4183, 57627, 81693, -20227, 7227, -1719, 191],
                ]
            )
            / 120960.0,
            np.array([-191, 1879, -9531, 68323, 68323, -9531, 1879, -191]) / 120960.0,
...
def cumulative_simpson(y, dx=1, initial=[]):
    return cumulative_composite(y, dx=dx, initial=initial, order=5)



def antiderivative1D(dxf, dx, C0, x0=0, debug=False):
    F = np.zeros(dxf.shape)

    F = np.roll(
        cumulative_simpson(np.roll(dxf, fd.ROLL_R * x0), dx, initial=0),
        fd.ROLL_L * x0,
    )
...
def antiderivative2D(dxf, dyf, dx, C0, x0=0, y0=0, debug=False):
    F = np.zeros(dxf.shape)

    F[y0, :] = np.roll(
        cumulative_simpson(np.roll(dxf[y0, :], fd.ROLL_R * x0), dx, initial=0),
        fd.ROLL_L * x0,
    )
    ...
    for i in range(F.shape[1]):
        F[:, i] = np.roll(
            cumulative_simpson(
                np.roll(dyf[:, i], fd.ROLL_R * y0), dx, initial=F[y0, i]
            ),
            fd.ROLL_L * y0,
        )
