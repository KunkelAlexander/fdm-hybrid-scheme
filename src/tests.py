...
def generate1DGaussian(x0, x, t, alpha=1.0 / 10):
    psi = np.sqrt(1 / (alpha + 1.0j * t)) * np.exp(
        -((x - x0) ** 2) / (2 * (alpha + 1.0j * t))
    )
    return psi

def li1(x, dx, t, x0=0.5, alpha= 1/20, eps=0):
    psi = eps + 0j
...
def periodicLi1(x, dx, t, x0=.5, alpha=1.0 / 10, eps=2, L=1, N=100):
    ...
    for i in range(-N, N + 1):
        psi += generate1DGaussian(x0, x + L * i, t, alpha)
    return normalise(psi)


def li2(x, dx, t, x0 = 1, A=np.sqrt(5)-np.sqrt(6), B=np.sqrt(6)):
    C, S = sc.fresnel((x-x0)*np.sqrt(1/(np.pi*t)))
    psi = A/2 + B - A/2*(1+1.j)*C - A/2*(1-1.j)*S
    return psi

def periodiLi2(x, dx, t, A=np.sqrt(5)-np.sqrt(6), B=np.sqrt(6)):
    psi = 0
    N = 500
    L = 1
    for i in range(-N, N + 1):
      psi += li2(x + L * i, dx, t, A, B)
    return psi


# Li test 3
def li3(x, dx, t, alpha= 1/500, k= 20*np.pi, x0=.5, x1=.1):
    C = np.sqrt(alpha / (alpha + 1.0j * t))
    psi = (
        C
        * np.exp(-((x + x1 - x0 - 1.0j * k * alpha) ** 2) / (2 * (alpha + 1.0j * t)))
        * np.exp(-(alpha * k ** 2) / 2)
    )
    psi += (
        C
...
def periodicLi3(x, dx, t, alpha=1 / 50, k=1 * np.pi, x0=5, x1=2, eps=2):
    psi = eps + 0j
    N = 500
    L = 10
    for i in range(-N, N + 1):
        psi += li3(x + L * i, dx, t, alpha, k, x0, x1)
...
# Li test 3
def travellingWavePacket(x, dx, t, alpha= 1/500, k= 20*np.pi, x0=.2):
    C = np.sqrt(alpha / (alpha + 1.0j * t))
    psi = (
        C
        * np.exp(-((x - x0 - 1.0j * k * alpha) ** 2) / (2 * (alpha + 1.0j * t)))
        * np.exp(-(alpha * k ** 2) / 2)
    )
    return normalise(psi)


def standingWave(xx, dx, t, k = 1):
    omega = k**2/2
...
def oscillatorEigenstate1D(xx, dx, t, n = 0, x0 = 3, m = 1, w = 1, hbar = 1):
    x = (xx-x0)
    xi = np.sqrt(m*w/hbar)*x
...
def oscillatorCoherentState1D(xx, dx, t, alpha = 1, x0 = 7, m = 1, w = 1, hbar = 1):
...
def oscillatorPotential1D(xx, x0 = 7, w = 1):
    return w/2 * (xx - x0)**2

def infiniteWell1D(xx, dx, t, n = 0, x0 = 0, L = 1, xl=0, xr=1, m = 1, w = 1, hbar = 1):
    kn = (n+1) * np.pi / L
...
def wellPotential1D(xx, xl = 0, xr = 1, V = 1e10):
    return (xx < xl) * V + (xx > xr) * V

def cosmological1D(x, dx, t, Lx=1, N=3, eps = 5e-3):
    psi = 1 + 0j
    for n in range(N):
        kx = 2 * np.pi * (1 + n) / Lx
        psi += eps * coefficients_1d[0, n] * np.cos(kx * x) * np.exp(-1j * t * kx ** 2 / 2)
        psi += eps * coefficients_1d[1, n] * np.sin(kx * x) * np.exp(-1j * t * kx ** 2 / 2)
...
@njit
def cosmological2D(x, y, dx, t, Lx=25, Ly=25, N=10, eps=5e-3):
...
# Generate analytical solution for the 1D free Schrödinger equation
# Li test 1
def generate2DGaussian(x0, y0, xx, yy, t, alpha=1.0 / 10):
    psi = np.sqrt(1 / (alpha + 1.0j * t)) * np.exp(
        -((xx - x0) ** 2 + (yy - y0) ** 2) / (2 * (alpha + 1.0j * t))
    )
    return normalise(psi)

def periodic2DGaussian(xx, yy, dx, t, x0=5, y0=5, alpha=1.0 / 10, eps=2, L=10, N=10):
    psi = eps + 0j
    for i in range(-N, N + 1):
...
def twoWavePackets2D(xx, yy, dx, t = 0, alpha = 1/10, eps = 2):
    psi = eps + 0j
    for i in range(-N, N + 1):
        for j in range(-N, N + 1):
...
def generateLiTest13D(x, y, z, dx, t = 0, x0 = 5, alpha = 1./50, eps = 10):
  psi = eps
  psi += generate3DGaussian(x0, x0, x0, x, y, z, t, alpha, eps)
  return normalise(psi)


def twoWavePackets3D(x, y, z, dx, t = 0, x0 = 3, x1 = 7, alpha = 1./2, eps = 10):
  psi = eps
...
@njit
def cosmological3D(x, y, z, dx, t, Lx=1, Ly=1, Lz=1, N=10, eps = 5e-3):
    psi = np.ones(x.shape, dtype=np.complex128)
    for n in range(N):
        for m in range(N):
          for l in range(N):
